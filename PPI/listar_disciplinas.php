...
if (isset($_POST['update_disciplina'])) {
    ...
    // Atualizar a disciplina com o novo nome, curso, docente e turma
    $stmt = $conn->prepare("UPDATE disciplinas SET nome = ?, curso_id = ?, docente_id = ?, turma_id = ? WHERE id = ?");
    $stmt->bind_param("siiii", $nome, $curso_id, $docente_id, $turma_id, $id);
...
// Consulta SQL para listar disciplinas com os nomes dos cursos, docentes e turmas
$sql = "
SELECT d.id, d.nome AS disciplina_nome, c.nome AS curso_nome, 
       GROUP_CONCAT(DISTINCT doc.nome SEPARATOR ', ') AS docente_nome, 
       GROUP_CONCAT(DISTINCT t.numero SEPARATOR ', ') AS turma_nome
FROM disciplinas d
JOIN cursos c ON d.curso_id = c.id
JOIN docentes_disciplinas dd ON dd.disciplina_id = d.id
JOIN docentes doc ON doc.id = dd.docente_id
JOIN turmas_disciplinas td ON td.disciplina_id = d.id
JOIN turmas t ON t.numero = td.turma_numero AND t.ano = td.turma_ano AND t.ano_ingresso = td.turma_ano_ingresso
GROUP BY d.id
";
...
// Obter lista de cursos, docentes e turmas para os seletores nos modais
$cursos = [];
...
$turmas = [];
$stmt = $conn->prepare("SELECT numero, ano_ingresso FROM turmas");
$stmt->execute();
$stmt->bind_result($turma_id, $turma_nome);
while ($stmt->fetch()) {
    $turmas[] = ['id' => $turma_id, 'nome' => $turma_nome]; // Corrigido para 'turma_nome'
}
$stmt->close();
                        ...
                        <tbody>
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($row['turma_nome']); ?></td>
                                    <td>
                                        <!-- Botões de ação -->
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <select name="turma_id" class="form-select" required>
                                                            ...
                                                            <?php foreach ($turmas as $turma): ?>
                                                                <option value="<?php echo $turma['id']; ?>" <?php echo ($turma['id'] == $row['turma_id']) ? 'selected' : ''; ?>>
